...
// Today + N days, as an ISO date string — used to prefill "next due date" as
// 30 days out the moment a seat is marked paid (still editable afterward).
        ...
        <label className="flex items-center gap-2.5 cursor-pointer mb-3">
          <input
            ...
            onChange={(e) => {
              ...
              if (e.target.checked) setDueDate(addDaysISO(30));
            }}
            className="accent-[#E8A33D] w-4 h-4"
          />
          <span className="text-[14px] text-[#EDE6D6]">Record a payment today</span>
        </label>
        ...
        {markingPaidNow && (
          <>
            ...
            <p className="text-[#7C8698] text-[11px] mt-2">
              Next due date set to {formatDueDate(dueDate)} (30 days out) — edit it above if needed. Saved when you hit Save below.
            </p>
          </>
        )}
      </div>
        ...
        <PillButton
          ...
          onClick={() => {
            ...
            onSave(seatId, {
              ...
              dueDate,
              paidAt: markingPaidNow ? new Date().toISOString() : seat.paidAt || "",
              depositAmount,
              depositStatus,
              aadhaarFront,
              aadhaarBack,
...
export default function App() {
  ...
  const handleDownloadPdf = () => {
    ...
    autoTable(doc, {
      startY: 62,
      head: [["Room", "Seat", "Status", "Occupant", "Phone", "Fee", "Payment", "Mode", "Due date", "Deposit", "Aadhaar no.", "Address"]],
      body: rows,
  ...
  const handleDownloadVacatePdf = () => {
    ...
    autoTable(doc, {
      startY: 62,
      head: [["Room", "Seat", "Occupant", "Phone", "Fee", "Payment", "Due date", "Deposit", "Deposit status", "Vacated on", "Aadhaar no.", "Address"]],
      body: rows,
