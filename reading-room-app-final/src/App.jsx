          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (e.target.checked) {
                ...
                // Anchored to the *previous* due date when there is one — a
                // renewal rolls the cycle forward by 30 days regardless of
                // which day the payment actually lands on. Only a brand-new
                // registration (no prior due date) falls back to 30 days
                // from the payment date itself.
                setDueDate(addDaysISO(30, seat.dueDate || payDate));
              }
            }}
          ...
          <>
            <Field
              ...
              onChange={(e) => {
                ...
                setPaymentDate(v);
                // Only recompute automatically when there's no prior due
                // date to anchor to (i.e. this is a first payment) — for a
                // renewal the next due date stays tied to the last due date.
                if (!seat.dueDate && v) setDueDate(addDaysISO(30, v));
              }}
            />
            ...
            <p className="text-[#7C8698] text-[11px] mt-2">
              {seat.dueDate
                ? `Next due date set to ${formatDueDate(dueDate)} — 30 days from the last due date (${formatDueDate(
                    seat.dueDate
                  )}). Edit it above if needed.`
                : `Next due date set to ${formatDueDate(dueDate)} — 30 days from the payment date. Edit it above if needed.`}{" "}
