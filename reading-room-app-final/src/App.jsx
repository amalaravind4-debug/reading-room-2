...
async function saveAppState({ rooms, selectedRoomId, reminderTemplate }) {
...
function SeatModal({ seatId, seat, roomName, reminderTemplate, onClose, onSave, onClear, onPaymentReceived }) {
  ...
  // "Present due date" — the due date currently on file (correctable if it
  // was wrong). Only meaningful once a seat is occupied.
  ...
  const [nextDueDate, setNextDueDate] = useState(seat.dueDate || "");
  ...
  // Always computed live from the next due date — never set by hand.
  const statusMeta = getPaymentDisplay(nextDueDate);
  ...
  return (
    <Modal onClose={onClose}>
      ...
      {seat.status === "occupied" && (
        <Field
          label="Present due date"
          type="date"
          value={currentDueDate}
          onChange={(e) => {
            const v = e.target.value;
            setCurrentDueDate(v);
            setNextDueDate(addDaysISO(cycleDays, v || paymentDate || undefined));
          }}
        />
      )}
      <Field
        ...
        onChange={(e) => {
          ...
          setCycleDays(days);
          setNextDueDate(addDaysISO(days, currentDueDate || paymentDate || undefined));
        }}
      />
      <Field
        label={seat.status === "occupied" ? "Next due date" : "Due date"}
        type="date"
        value={nextDueDate}
      ...
      <div className="mb-6">
        ...
        >
          ...
          <span className="text-[#5B6472] text-[12px] ml-auto">based on the next due date above</span>
        </div>
      </div>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (e.target.checked) {
                ...
                // Anchored to the present due date when there is one — a
                // renewal rolls the cycle forward by the chosen number of
                // days regardless of which day the payment actually lands
                // on. Only a brand-new registration (no present due date)
                // falls back to the cycle length from the payment date.
                setNextDueDate(addDaysISO(cycleDays, currentDueDate || payDate));
              }
            }}
          ...
          <>
            <Field
              ...
              onChange={(e) => {
                const v = e.target.value;
                setPaymentDate(v);
                // Only recompute automatically when there's no present due
                // date to anchor to (i.e. this is a first payment) — for a
                // renewal the next due date stays tied to the present one.
                if (!currentDueDate && v) setNextDueDate(addDaysISO(cycleDays, v));
              }}
            />
            ...
            <p className="text-[#7C8698] text-[11px] mt-2">
              {currentDueDate
                ? `Next due date set to ${formatDueDate(nextDueDate)} — ${cycleDays} day${
                    cycleDays === 1 ? "" : "s"
                  } from the present due date (${formatDueDate(currentDueDate)}). Edit either field above if needed.`
                : `Next due date set to ${formatDueDate(nextDueDate)} — ${cycleDays} day${
                    cycleDays === 1 ? "" : "s"
                  } from the payment date. Edit it above if needed.`}{" "}
              Saved when you hit Save below.
            </p>
          </>
        )}
      </div>
        ...
        <PillButton
          ...
          onClick={() => {
            ...
            onSave(seatId, {
              ...
              phone,
              fee,
              paymentStatus: getPaymentStatusKey(nextDueDate),
            ...
            if (markingPaidNow) {
              onPaymentReceived({
                ...
                dueDateAtPayment: currentDueDate || seat.dueDate || "",
                nextDueDate,
              });
            }
          }}
...
export default function App() {
  ...
  const handleDownloadPdf = () => {
    ...
    autoTable(doc, {
      startY: 62,
      head: [
        ["Room", "Seat", "Status", "Occupant", "Phone", "Fee", "Payment", "Mode", "Payment date", "Due date", "Deposit", "Aadhaar no.", "Address"],
      ],
      body: rows,
  ...
  const handleDownloadPaymentReportPdf = () => {
    ...
    autoTable(doc, {
      startY: 62,
      head: [["Room", "Seat", "Occupant", "Amount", "Mode", "Paid on", "Due paid off", "Next due"]],
      body: rows,
  ...
  useEffect(() => {
    ...
    saveAppState({ rooms: cleanRooms, selectedRoomId, reminderTemplate })
      .then(() => {
  ...
  }, [rooms, selectedRoomId, reminderTemplate, hydrated]);
                  ...
                  <table className="w-full text-[14px]">
                    <thead>
                      <tr className="border-b border-[#2A2F3A] text-[#7C8698] text-[11px] uppercase tracking-wider">
                        ...
                        <th className="text-left px-4 py-3 font-medium hidden sm:table-cell">Due paid off</th>
                        <th className="text-left px-4 py-3 font-medium hidden sm:table-cell">Next due</th>
