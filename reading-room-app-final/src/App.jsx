...
// Drops any payment record older than the retention window, deleting it from
// Supabase (best-effort) and returning what's left. Only ever touches
// payment_log rows by paid_at — never rewrites a seat's live data.
...
export default function App() {
  ...
  const reminderList = useMemo(
    () =>
      allOccupants
        // A seat with paidAt + a still-pending nextDueDate has already paid for
        // the upcoming cycle — dueDate just hasn't rolled over yet (that only
        // happens at the start of next month). Match getPaymentDisplay's logic
        // so a seat that shows "Paid" in the UI doesn't also generate a reminder.
        .filter((o) => !(o.paidAt && o.nextDueDate && daysUntilDue(o.nextDueDate) >= 0))
