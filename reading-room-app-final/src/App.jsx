...
function SeatModal({ seatId, seat, roomName, reminderTemplate, onClose, onSave, onClear, onPaymentReceived }) {
  ...
  // The due date on file. Recording a payment below no longer changes this —
  // it only stays editable here for manual correction (e.g. fixing a typo,
  // or setting the very first due date at registration). Due dates only
  // move automatically once a month, via the renewal batch in the Reminders
  // tab, for whoever paid the month before.
        ...
        {markingPaidNow && (
          <>
            ...
            <p className="text-[#7C8698] text-[11px] mt-2">
              This won't change the due date above — it only logs that the payment came in. Due dates renew
              automatically once a month for whoever paid last month (set that up in the Reminders tab).
            </p>
          </>
        )}
      </div>
          ...
          onClick={() => {
            ...
            if (markingPaidNow) {
              onPaymentReceived({
                ...
                // The due date doesn't move at payment time anymore — both
                // columns just record what it was on file when this payment
                // came in, for the historical receipt.
...
function RoomBoard({ room, onSeatUpdate, editMode, onToggleEdit, onAddRow, onAddCol, onVacateSeat, reminderTemplate, onPaymentReceived }) {
  ...
  const counts = useMemo(() => {
    ...
    Object.values(room.seats).forEach((s) => {
      ...
      if (s.status === "vacant") vacant++;
        ...
        occupied++;
        if (getPaymentStatusKey(s.dueDate) === "overdue") overdue++;
      }
    });
  ...
  const seatMatchesFilter = (seat) => {
    ...
    return seat.status === "occupied" && getPaymentStatusKey(seat.dueDate) === filter;
  };
...
export default function App() {
  ...
  const handleDownloadPdf = () => {
      ...
      Object.entries(rm.seats)
        ...
        .forEach(([seatId, s]) => {
          rows.push([
            ...
            s.status === "occupied" ? getPaymentDisplay(s.dueDate).label : "—",
    ...
    autoTable(doc, {
      ...
      foot: [
        [
          {
            content: `* Next due date is a projection — ${renewalCycleDays} days from the present due date, applied automatically only if this seat pays during the current month.`,
            colSpan: 14,
          },
        ],
      ],
  ...
  // Payments within the chosen From/To range (inclusive). Empty = no bound.
  const filteredPaymentLog = useMemo(() => {
    return paymentLog.filter((r) => {
  ...
  }, [paymentLog, paymentLogFrom, paymentLogTo]);
  ...
  const handleDownloadPaymentReportPdf = () => {
    ...
    const rows = filteredPaymentLog.map((r) => [
      ...
      // A live projection (present due date + the current renewal cycle
      // length), not the stored nextDueDate — recording a payment no longer
      // moves the due date on its own, so the two are otherwise identical.
      r.dueDateAtPayment ? formatDueDate(addDaysISO(renewalCycleDays, r.dueDateAtPayment)) : "—",
    ]);
    ...
    autoTable(doc, {
      startY: 62,
      head: [["Room", "Seat", "Occupant", "Amount", "Mode", "Payment date", "Present due date", "Next due date*"]],
      body: rows,
      styles: { fontSize: 8.5, cellPadding: 5 },
      headStyles: { fillColor: [20, 23, 28], textColor: 255 },
      alternateRowStyles: { fillColor: [245, 245, 245] },
      foot: [
        ["", "", "", `₹${paymentLogTotal}`, "", "Total received", "", ""],
        [
          {
            content: `* Next due date is a projection — ${renewalCycleDays} days from the present due date, applied automatically only if this seat pays during the current month.`,
            colSpan: 8,
      ...
      didParseCell: (data) => {
        // The footnote row shouldn't inherit the bold gold "total" styling.
  ...
  // Payment log: loaded once on mount, added to every time a seat is newly
  // marked paid (see SeatModal's onPaymentReceived).
  useEffect(() => {
    if (!SUPABASE_CONFIGURED) return;
    let cancelled = false;
    fetchPaymentLog().then((rows) => {
      if (!cancelled) setPaymentLog(rows);
    });
    return () => {
      cancelled = true;
    };
  ...
  const addPaymentRecord = (record) => {
    setPaymentLog((prev) => [record, ...prev]);
  ...
  // due date by itself — it only logs that a payment happened. This is the
  // one place due dates actually move: for every occupied seat that paid
  // during the *previous* calendar month, the due date rolls forward by
  // `renewalCycleDays`. Anyone who didn't pay last month is left untouched
  // (so they simply stay showing overdue until they do pay).
  useEffect(() => {
    if (!hydrated) return;
    ...
    setRooms((prev) =>
      prev.map((room) => {
        ...
        Object.entries(room.seats || {}).forEach(([sid, s]) => {
          if (s.status === "occupied" && s.paidAt && s.paidAt.slice(0, 7) === prevMonth) {
            changed = true;
            nextSeats[sid] = { ...s, dueDate: addDaysISO(renewalCycleDays, s.dueDate) };
          } else {
            nextSeats[sid] = s;
          }
        });
    ...
    setLastRenewalMonth(thisMonth);
  }, [hydrated, lastRenewalMonth, renewalCycleDays]);
  ...
  const analytics = useMemo(() => {
    ...
    rooms.forEach((rm) => {
      Object.values(rm.seats).forEach((s) => {
        ...
        occupied++;
        const fee = s.fee || 0;
        const statusKey = getPaymentStatusKey(s.dueDate);
            ...
            <div>
              ...
              <div className="bg-[#181B22] border border-[#2A2F3A] rounded-2xl overflow-hidden">
                <table className="w-full text-[14px]">
                  ...
                  <tbody>
                    {allOccupants.map((o) => {
                      const meta = getPaymentDisplay(o.dueDate);
                      return (
                  ...
                  <table className="w-full text-[14px]">
                    <thead>
                      <tr className="border-b border-[#2A2F3A] text-[#7C8698] text-[11px] uppercase tracking-wider">
                        ...
                        <th className="text-left px-4 py-3 font-medium hidden sm:table-cell">Present due date</th>
              ...
              {filteredPaymentLog.length > 0 && (
                <p className="text-[#5B6472] text-[11px] mt-2">
                  * Next due date is a projection — {renewalCycleDays} days from the present due date, applied
                  automatically only if the seat pays during the current month.
                </p>
              )}
            </div>
          )}
            ...
            <div>
              ...
              <div className="bg-[#181B22] border border-[#2A2F3A] rounded-2xl p-4 mb-5">
                ...
                <p className="text-[#7C8698] text-[12px] mb-3 leading-relaxed">
                  Recording a payment only marks it received — it never changes a seat's due date by itself. Instead,
                  the first time you open the app after a new calendar month begins, due dates roll forward
                  automatically by the number of days below, but only for seats that were paid during the previous
                  month. Anyone who didn't pay last month is left as-is.
                </p>
