...
- **Payments** — status (**Paid** / **Due soon** / **Overdue**) is computed automatically from the due date, not set by hand — comfortably ahead of the due date reads "Paid," within 3 days reads "Due soon," past it reads "Overdue." The **due date** is its own editable field on the seat, available at registration too. Ticking "Record a payment" (on assign or edit) reveals an editable **payment date** (defaults to today, can be backdated) and the **mode of payment** (Cash/UPI); the next due date auto-fills to 30 days out from the *last* due date — not from the payment date — so the billing cycle stays on schedule even if a payment lands early or late (still editable by hand either way). Every recorded payment creates a permanent receipt entry, viewable with a date-range filter, a running total, and a PDF export; the payment date can also be corrected after the fact from the Payments tab
...
- **ID documents** — Aadhaar number, address, and Aadhaar front/back + address-proof photos per occupant (all stored locally only, see above); a "Seat PDF" export downloads every seat's full details (occupant, payment status, payment mode, payment date, due date, deposit, Aadhaar number, address) as one table; a separate "ID docs" export bundles all uploaded photos into a zip named by occupant
